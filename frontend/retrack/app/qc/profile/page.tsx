...
export default function QCProfilePage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/users/${userId}/profile`)
      .then(res => {
        if (!res.ok) throw new Error("Failed to load QC profile");
        return res.json();
      })
    ...
    // load saved photo
    const savedPhoto = localStorage.getItem(`qc-profile-photo-${userId}`);
    if (savedPhoto) {
      setPhoto(savedPhoto);
    }
  ...
  const handlePhotoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      ...
      setPhoto(base64);
      localStorage.setItem(`qc-profile-photo-${userId}`, base64);
    };
    ...
    <div className="min-h-screen bg-gray-950 text-white px-6 py-10 flex justify-center">
      <div className="w-full max-w-4xl">
        ...
        <div className="flex justify-between mb-8">
          <h1 className="text-3xl font-bold">QC Profile</h1>
          <button
            onClick={() => router.back()}
